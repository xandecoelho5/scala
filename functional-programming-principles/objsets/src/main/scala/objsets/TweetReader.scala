...
object TweetReader:
  ...
  private object ParseTweets:
    private def regexParser(s: String): List[Map[String, Any]] =
  ...
  private def toTweetSet(l: List[Tweet]): TweetSet =
  ...
  private val sites = List("gizmodo", "TechCrunch", "engadget", "amazondeals", "CNET", "gadgetlab", "mashable")
  ...
  private val tweetSets: List[TweetSet] = sources.map(tweets => toTweetSet(tweets))
