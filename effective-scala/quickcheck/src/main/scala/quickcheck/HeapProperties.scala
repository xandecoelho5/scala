...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    "melding a heap containing two low values with a heap containing two high values" ->
      forAll { (x: Int, y: Int) =>
        ...
        val higher = math.max(x, y)
        val lower = math.min(x, y)
        val heap1 = insert(higher, insert(higher, empty))
        val heap2 = insert(lower, insert(lower, empty))
        ...
        // highest value
        val deleteTwoMinAndFindMin: Boolean = findMin(deleteMin(deleteMin(meldedHeap))) == higher

        // check that inserting the lowest value to the melded heap, and then
        // finding the minimal element returns the lowest value
        val insertMinAndFindMin: Boolean = findMin(insert(lower, meldedHeap)) == lower
  ...
  val meldingHeaps: (String, Prop) =
    @tailrec
    def check(m: List[Node], h1: List[Node], h2: List[Node]): Boolean =
      if isEmpty(m) then isEmpty(h1) && isEmpty(h2)
      else
        val minM = findMin(m)
        if h1.nonEmpty && findMin(h1) == minM then check(deleteMin(m), deleteMin(h1), h2)
        else if h2.nonEmpty && findMin(h2) == minM then check(deleteMin(m), h1, deleteMin(h2))
        else false
  ...
  given generatedHeap: Gen[List[Node]] = oneOf(
    const(empty),
    for
      v <- arbitrary[Int]
