...
object PersistentModel extends Model:
   ...
   */
  private def saveTasks(tasks: Tasks): Unit =
  ...
  def create(task: Task): Id =
    ...
    saveTasks(Tasks(List(id -> task)))
    saveId(id)
    id

  def read(id: Id): Option[Task] =
    loadTasks().tasks.find(t => t._1 == id).map(t => t._2)

  def update(id: Id)(f: Task => Task): Option[Task] =
    loadTasks().tasks.find(t => t._1 == id).
  ...
  def tasks(tag: Tag): Tasks =
    val filteredTasks = loadTasks().tasks.filter(a => a._2.tags.contains(tag))
    Tasks(filteredTasks)
  ...
  def tags: Tags =
    Tags(Set.from(loadTasks().tasks.flatMap(a => a._2.tags)).toList)
